...
namespace mml {
  ...
  class postfix_writer: public basic_ast_visitor {
    ...
    int _offset;
    bool _inFunctionBody, _inFunctionArgs, _returnSeen;
    std::string _fun_label;
    std::string _extern_label;

    std::deque<int> _whileCond, _whileEnd, _withCond, _withEnd;
  ...
  public:
    postfix_writer(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<mml::symbol> &symtab,
        ...
        basic_ast_visitor(compiler), _symtab(symtab), _pf(pf), _lbl(0), _offset(0),_inFunctionBody(false), _inFunctionArgs(false), _returnSeen(false) {
    }
