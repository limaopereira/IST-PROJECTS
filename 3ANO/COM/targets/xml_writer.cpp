...
void mml::xml_writer::do_with_node(mml::with_node * const node, int lvl) {
  /* ASSERT_SAFE_EXPRESSIONS; */
    openTag(node, lvl);
...
void mml::xml_writer::do_function_definition_node(mml::function_definition_node * const node, int lvl){
    os() << std::string(lvl, ' ') << "<" << node->label() << " type='" << type_name(node -> type()) << "' main=" << (node -> isMain() ? "true" : "false") << "'>" << std::endl;
