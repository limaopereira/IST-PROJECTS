...
Cypress.Commands.add('createCourseExecution', (name, acronym, academicTerm) => {
  ...
  Cypress.Commands.add('initializeDataCE', () => {
      const courseExecutions = {
          '1st Semester 2023/2024' : {
              created: false,
              studentStats: {
                  numAtLeast3Quizzes : 8,
                  numMore75CorrectQuestions : 3, 
                  numStudents : 20,
              },
              questionStats: {
                  answeredQuestionsUnique: 40,
                  averageQuestionsAnswered: 20,
                  numAvailable: 60,
              },
              quizStats: {
                  averageQuizzesSolved: 23,
                  numQuizzes: 140,
                  numUniqueAnsweredQuizzes: 28,
              },
              dependencies: ['1st Semester 2023/2024','1st Semester 2022/2023', '1st Semester 2019/2020'],
          },
          '1st Semester 2022/2023' : {
              created: false,
              studentStats: {
                  numAtLeast3Quizzes : 3,
                  numMore75CorrectQuestions : 20, 
                  numStudents : 8,
              },
              questionStats: {
                  answeredQuestionsUnique: 60,
                  averageQuestionsAnswered: 20,
                  numAvailable: 80,
              },
              quizStats: {
                  averageQuizzesSolved: 32,
                  numQuizzes: 150,
                  numUniqueAnsweredQuizzes: 82,
              },
              dependencies: ['1st Semester 2022/2023', '1st Semester 2019/2020'],
          },
          '1st Semester 2019/2020' : {
              created: false,
              studentStats: {
                  numAtLeast3Quizzes : 20,
                  numMore75CorrectQuestions : 8, 
                  numStudents : 3,
              },
              questionStats: {
                  answeredQuestionsUnique: 80,
                  averageQuestionsAnswered: 40,
                  numAvailable: 120,
              },
              quizStats: {
                  averageQuizzesSolved: 40,
                  numQuizzes:160,
                  numUniqueAnsweredQuizzes: 70,
              },
              dependencies: ['1st Semester 2019/2020'],
          },
      }
      
      cy.createDemoCourseExecution('1st Semester 2017/2018');
      cy.createTeacherDashboard('1st Semester 2017/2018');
      
      Object.keys(courseExecutions).forEach((ce) => {
          if (!courseExecutions[ce].created) {
            cy.createCourseExecutionOnDemoCourse(ce);
            cy.addTeacherToCourseExecution(ce);
            courseExecutions[ce].created = true;
          }
          cy.createTeacherDashboard(ce);
          courseExecutions[ce].dependencies.forEach((dep) => {
              if(!courseExecutions[dep].created){
                  cy.createCourseExecutionOnDemoCourse(dep);
                  cy.addTeacherToCourseExecution(dep);
                  courseExecutions[dep].created = true;
              }
              cy.createStudentStats(
                  courseExecutions[dep].studentStats.numAtLeast3Quizzes,
                  courseExecutions[dep].studentStats.numMore75CorrectQuestions,
                  courseExecutions[dep].studentStats.numStudents,
                  dep,
                  ce
              );
              
              cy.createQuestionStats(
                  courseExecutions[dep].questionStats.answeredQuestionsUnique,
                  courseExecutions[dep].questionStats.averageQuestionsAnswered,
                  courseExecutions[dep].questionStats.numAvailable,
                  dep,
                  ce
              );
  
              cy.addTeacherDashboardQuestionsStats(
                  ce
              );
              
              cy.createQuizStats(
                  courseExecutions[dep].quizStats.averageQuizzesSolved,
                  courseExecutions[dep].quizStats.numQuizzes,
                  courseExecutions[dep].quizStats.numUniqueAnsweredQuizzes,
                  dep,
                  ce
              );
  
              cy.addTeacherDashboardQuizStats(
                  ce
              );
          });
      })
  ...
  Cypress.Commands.add('checkCharts', (path,stat) => {
    const PNG = require('pngjs').PNG;
    const pixelmatch = require('pixelmatch');

    cy.readFile(
      'tests/e2e/screenshots/teacherDashboard.js/'+ path +'/result-screenshots/' + stat +'.png', 'base64'
    ).then(baseImage => {
      cy.readFile(
        'tests/e2e/specs/teacher/base-screenshots/'+ path + '/' + stat +'.png', 'base64'
      ).then(resultImage => {
				// load both pictures
        const img1 = PNG.sync.read(Buffer.from(baseImage, 'base64'));
        const img2 = PNG.sync.read(Buffer.from(resultImage, 'base64'));
