function dbPasswordCommand(password) {
  ...
  Cypress.Commands.add('createDemoCourseExecution', (academicTerm) =>{
      cy.task('queryDatabase', {
          query: `INSERT into course_executions (id, academic_term, acronym, end_date, status, type, course_id)
                  SELECT
                      COALESCE((SELECT max(id) FROM course_executions), 0) + 1,
                      '${academicTerm}',
                      'DemoCourse',
                      NULL,
                      'ACTIVE',
                      (SELECT type FROM courses WHERE name = 'Demo Course'),
  ...
  Cypress.Commands.add('addTeacherToCourseExecution', (academicTerm) =>{
      cy.task('queryDatabase', {
  ...
  Cypress.Commands.add('cleanDataCE', () => {
      dbCommand(`
             DELETE FROM student_dashboard_removed_difficult_questions;
             ...
             DELETE FROM users_course_executions
                  WHERE course_executions_id NOT IN (
                      SELECT course_id FROM course_executions WHERE academic_term = '1st Semester 2017/2018'
                  );
  ...
  Cypress.Commands.add('createTeacherDashboard', (academicTerm) => {
      cy.task('queryDatabase', {
          query: `INSERT into teacher_dashboard (id, course_execution_id, teacher_id)
                  SELECT
                      COALESCE((SELECT max(id) FROM teacher_dashboard), 0) + 1,
                      (SELECT id FROM course_executions WHERE academic_term = '${academicTerm}'),
                      (SELECT id FROM users WHERE name = 'Demo Teacher')`,
          credentials: credentials,
      });
  ...
  Cypress.Commands.add('createStudentStats', (numAtLeast3Quizzes, numMore75CorrectQuestions, numStudents, academicTermCE, academicTermTD) => {
      cy.task('queryDatabase', {
          query: `INSERT into student_stats (id, num_at_least3quizzes, num_more75correct_questions, num_students, course_execution_id, teacher_dashboard_id)
                  SELECT 
                      COALESCE((SELECT max(id) FROM student_stats),0) + 1,
                      '${numAtLeast3Quizzes}',
                      '${numMore75CorrectQuestions}',
                      '${numStudents}',
                      (SELECT id FROM course_executions WHERE academic_term = '${academicTermCE}'),
                      td.id
                  FROM course_executions ce
                  JOIN teacher_dashboard td ON ce.id = td.course_execution_id
                  WHERE ce.academic_term = '${academicTermTD}'`,
          credentials: credentials,
      });
  });
  
  
  Cypress.Commands.add('createQuestionStats', (answeredQuestionsUnique, averageQuestionsAnswered, numAvailable, academicTermCE, academicTermTD) =>{
      cy.task('queryDatabase', {
          query: `INSERT into question_stats (id, answered_questions_unique, average_questions_answered, num_available, dashboard_id, execution_id)
                  SELECT 
                      COALESCE((SELECT max(id) FROM question_stats),0) + 1,
  ...
  Cypress.Commands.add('addTeacherDashboardQuestionsStats', (academicTermTD) => {
      cy.task('queryDatabase', {
          query: `INSERT into teacher_dashboard_question_stats (teacher_dashboard_id, question_stats_id)
                  SELECT
                      td.id, 
                      COALESCE((SELECT max(question_stats_id) FROM teacher_dashboard_question_stats),0) + 1
                  FROM course_executions ce
                  JOIN teacher_dashboard td ON ce.id = td.course_execution_id
                  WHERE ce.academic_term = '${academicTermTD}'`,
          credentials: credentials,
  ...
  Cypress.Commands.add('createQuizStats', (averageQuizzesSolved, numQuizzes, numUniqueAnsweredQuizzes, academicTermCE, academicTermTD) => {
      cy.task('queryDatabase', {
          query: `INSERT into quiz_stats (id, average_quizzes_solved, num_quizzes, num_unique_answered_quizzes, course_execution_id, teacher_dashboard_id)
                  SELECT 
                      COALESCE((SELECT max(id) FROM quiz_stats),0) + 1,
  ...
  Cypress.Commands.add('addTeacherDashboardQuizStats', (academicTermTD) => {
      cy.task('queryDatabase', {
          query: `INSERT into teacher_dashboard_quiz_stats (teacher_dashboard_id, quiz_stats_id)
                  SELECT
                      td.id, 
                      COALESCE((SELECT max(quiz_stats_id) FROM teacher_dashboard_quiz_stats),0) + 1
                  FROM course_executions ce
                  JOIN teacher_dashboard td ON ce.id = td.course_execution_id
                  WHERE ce.academic_term = '${academicTermTD}'`,
          credentials: credentials,
