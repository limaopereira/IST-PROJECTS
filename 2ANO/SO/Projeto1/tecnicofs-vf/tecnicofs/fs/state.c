...
// static locks inode_locks[INODE_TABLE_SIZE];

static inode_t inode_table[INODE_TABLE_SIZE];
static pthread_rwlock_t locks_inode_table[INODE_TABLE_SIZE];
static char freeinode_ts[INODE_TABLE_SIZE]; // trinco global
static pthread_mutex_t lock_freeinode_ts;

// inode_locks[inumber].lock()
// inode_locks[inumber].unlock()

/* Data blocks */
static char fs_data[BLOCK_SIZE * DATA_BLOCKS];
//static pthread_mutex_t locks_fs_data[BLOCK_SIZE * DATA_BLOCKS];
static char free_blocks[DATA_BLOCKS]; // trinco global
static pthread_mutex_t lock_free_blocks;

/* Volatile FS state */

static open_file_entry_t open_file_table[MAX_OPEN_FILES];
static pthread_mutex_t locks_open_file_table[MAX_OPEN_FILES];
static char free_open_file_entries[MAX_OPEN_FILES]; // trinco global
static pthread_mutex_t lock_free_open_file_entries;
...
void state_init() {
    ...
    // Devemos verificar erros na inicialização dos mutexes?

    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        freeinode_ts[i] = FREE;
        pthread_rwlock_init(&locks_inode_table[i],NULL);
    }
    ...
    for (size_t i = 0; i < MAX_OPEN_FILES; i++) {
        ...
        pthread_mutex_init(&locks_open_file_table[i],NULL);
    }

    pthread_mutex_init(&lock_freeinode_ts,NULL);
    pthread_mutex_init(&lock_free_blocks,NULL);
    pthread_mutex_init(&lock_free_open_file_entries,NULL);
}
...
void state_destroy() {
    /* nothing to do */
    for(size_t i=0;i<INODE_TABLE_SIZE;i++)
        pthread_rwlock_destroy(&locks_inode_table[i]);
    for(size_t i=0;i<MAX_OPEN_FILES;i++)
        pthread_mutex_destroy(&locks_open_file_table[i]);
    pthread_mutex_destroy(&lock_freeinode_ts);
    pthread_mutex_destroy(&lock_free_blocks);
    pthread_mutex_destroy(&lock_free_open_file_entries);
}
...
int inode_create(inode_type n_type) {
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        pthread_mutex_lock(&lock_freeinode_ts);
        if (freeinode_ts[inumber] == FREE) {
            /* Found a free entry, so takes it for the new i-node*/
            freeinode_ts[inumber] = TAKEN;
            pthread_mutex_unlock(&lock_freeinode_ts);
            ...
            if (n_type == T_DIRECTORY) {
                ...
                for(int i=0;i<11;i++)
                    inode_table[inumber].i_data_block[i]=-1; // Alguma forma de melhorar isto?
            }
            inode_unlock(inumber);
            return inumber;
        }
...
void inode_wrlock(int inumber){
    pthread_rwlock_wrlock(&locks_inode_table[inumber]);
}


void inode_rdlock(int inumber){
    pthread_rwlock_rdlock(&locks_inode_table[inumber]);
}


void inode_unlock(int inumber){
    pthread_rwlock_unlock(&locks_inode_table[inumber]);
}
...
int data_block_alloc() {
    for (int i = 0; i < DATA_BLOCKS; i++) {
        ...
        pthread_mutex_lock(&lock_free_blocks);
        if (free_blocks[i] == FREE) {
            free_blocks[i] = TAKEN;
            pthread_mutex_unlock(&lock_free_blocks);
            return i;
        }
        pthread_mutex_unlock(&lock_free_blocks);
    }
...
int add_to_open_file_table(int inumber, size_t offset, int flags) {
    ...
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_mutex_lock(&lock_free_open_file_entries);
        if (free_open_file_entries[i] == FREE) {
            free_open_file_entries[i] = TAKEN;
            file_lock(i);
            pthread_mutex_unlock(&lock_free_open_file_entries);
            open_file_table[i].of_inumber = inumber;
            open_file_table[i].of_offset = offset;
            open_file_table[i].of_flags= flags;
            file_unlock(i);
            return i;
        }
        pthread_mutex_unlock(&lock_free_open_file_entries);
...
void file_lock(int fhandle){
    pthread_mutex_lock(&locks_open_file_table[fhandle]);
}


void file_unlock(int fhandle){
    pthread_mutex_unlock(&locks_open_file_table[fhandle]);
}
