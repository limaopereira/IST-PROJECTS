...
int tfs_destroy() {
    state_destroy();
    if(pthread_mutex_unlock(&single_global_lock)!=0) // Ter atenção a isto!
        return -1;
...
int tfs_destroy_after_all_closed() {
    if(pthread_mutex_lock(&single_global_lock)!=0)
        return -1;
    while(get_number_open_files()!=0){
        if(pthread_cond_wait(&cond,&single_global_lock)!=0){
            if(pthread_mutex_unlock(&single_global_lock)!=0) // É necessário fazer esta verificação?
                return -1;
            return -1;
        }
    }
